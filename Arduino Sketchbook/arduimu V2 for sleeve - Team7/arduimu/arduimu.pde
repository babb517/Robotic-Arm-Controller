...
#define PRINT_BINARY 0  //Will print binary message and suppress ASCII messages (above) CHANGED FROM 1
...
{ 
  /////////////////////////////
  // ALWAYS DOUBLE CHECK BOARD SETTINGS UNDER TOOLS
  // WHEN UPLOADING CODE TO THE ARDUIMU!!!
  // BOARD SETTING: DUEMILANOVE W/ ATMEGA328
  /////////////////////////////
  //Serial.begin(28800, 128, 16); // When hooked up the Lilypad
  
  Serial.begin(38400, 128, 16); // For stand alone
...
void loop() //Main Loop
{
  ...
  // hundred readings per second (10); perhaps could even make it faster? But be careful as you add more processing; may need to adjust
  if((timeNow-timer)>=10)//25)//50)//100)//20  // Main loop runs at 50Hz
  {
    timer_old = timer;
    timer = timeNow;
	...
	cycleCount++;
		...
		switch (cycleCount) {
			...
			case(5):
				...
				//CUSTOM ADDITION
				//the IF statement prevents data from being sent to the lilypad when gyros are saturated -- remove for debugging or tests
				if (gyro_sat < 1) {
					#if !PRINT_BINARY
						printdata(); //Send info via serial
					#endif
